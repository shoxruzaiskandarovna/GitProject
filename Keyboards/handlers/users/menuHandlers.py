...
from keyboards.inline.productsKeyboards import categoryMenu, coursesMenu, booksMenu, telegram_keyboard, algoritm
...
async def select_category(message: Message):
    ...
    await message.answer(text=f"Mahsulot tanlang", reply_markup=categoryMenu)
...
async def buy_courses(call: CallbackQuery):
    ...
    await call.message.answer(text="Kurs tanlang", reply_markup=coursesMenu)
...
async def buy_books(call: CallbackQuery):
    ...
    logging.info(f"{callback_data=}")
    # await call.message.edit_reply_markup(reply_markup=None)
    await call.message.answer(text="Kitoblar", reply_markup=booksMenu)
    await call.message.delete()
    await call.answer(cache_time=60)
...
async def buying_course(call: CallbackQuery, callback_data: dict):
    ...
    await call.message.answer(text=f"Siz Mukammal Telegram Bot Kursini tanladingiz.",
...
async def buying_book(call: CallbackQuery, callback_data: dict):
    logging.info(f"{callback_data=}")
    await call.answer(text="Buyurtmangiz qabul qilindi", cache_time=60, show_alert=True)